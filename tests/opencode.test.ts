...
describe("adaptTheme", () => {
  ...
  it("maps the 2.0.8 raised backgrounds when surface is gone", () => {
  ...
  it("never throws on a partial theme and falls back to the text color", () => {
    const adapted = adaptTheme({} as ResolvedTheme);

    expect(adapted.text).toBe("#ffffff");
    expect(adapted.backgroundPanel).toBe("#ffffff");
    expect(adapted.markdownHeading).toBe("#ffffff");
    expect(adapted.syntaxKeyword).toBe("#ffffff");
  });
});
